...
context(`REST Deploy sourcepath NUTs [name: ${REPO.gitUrl}] [exec: ${EXECUTABLE} ]`, () => {
  ...
  describe('--sourcepath flag', () => {
    const toDeploy = path.normalize('"force-app, my-app, foo-bar"');
    it(`should deploy ${toDeploy}`, async () => {
      await testkit.deploy({ args: `--sourcepath ${toDeploy}` });
      await testkit.expect.filesToBeDeployed(['force-app/**/*', 'my-app/**/*', 'foo-bar/**/*']);
    });
  });
});
