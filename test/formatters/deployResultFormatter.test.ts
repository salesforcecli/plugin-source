...
describe('DeployResultFormatter', () => {
  ...
  describe('getJson', () => {
    ...
    it('should return expected json for a failure', () => {
      ...
      const formatter = new DeployResultFormatter(ux, {}, deployResultFailure);
    ...
    describe('replacements', () => {
      it('includes expected json property when there are replacements', () => {
        ...
        const formatter = new DeployResultFormatter(ux, {}, resultWithReplacements as DeployResult);
        const json = formatter.getJson();
      ...
      it('omits json property when there are no replacements', () => {
        ...
        const formatter = new DeployResultFormatter(ux, {}, resultWithoutReplacements as DeployResult);
        const json = formatter.getJson();
      ...
      it('omits json property when replacements exists but is empty', () => {
        ...
        const formatter = new DeployResultFormatter(ux, {}, resultWithEmptyReplacements as DeployResult);
        const json = formatter.getJson();
        expect(json.replacements).to.be.undefined;
      });
    });
  ...
  describe('display', () => {
    ...
    it('should output as expected for a deploy failure (GACK)', () => {
      ...
      const formatter = new DeployResultFormatter(ux, {}, deployFailure);
    ...
    it('should output as expected for passing tests with verbose', () => {
      const formatter = new DeployResultFormatter(ux, { verbose: true }, deployResultTestSuccess);
      formatter.display();
      expect(styledHeaderStub.calledThrice).to.equal(true);
      expect(logStub.callCount).to.equal(8);
      expect(tableStub.calledThrice).to.equal(true);
    ...
    it('should output as expected for passing and failing tests with verbose', () => {
      const formatter = new DeployResultFormatter(ux, { verbose: true }, deployResultTestSuccessAndFailure);
      formatter.display();
    ...
    it('shows success AND failures for partialSucceeded', () => {
      const formatter = new DeployResultFormatter(ux, { verbose: true }, deployResultPartialSuccess);
      formatter.display();
    ...
    describe('replacements', () => {
      it('omits replacements when there are none', () => {
        ...
        const formatter = new DeployResultFormatter(ux, { verbose: true }, resultWithoutReplacements);
      ...
      it('omits replacements unless verbose', () => {
        ...
        const formatter = new DeployResultFormatter(ux, {}, resultWithReplacements);
        formatter.display();
